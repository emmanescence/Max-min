...
def display_tickers(tickers):
    ...
    for ticker in tickers:
        ...
        if drawdown is not None:
            results.append({
                'Ticker': ticker,
                'Caída desde máx (%)': drawdown,  # Mantén un solo decimal
    ...
    df_sorted = df.sort_values('Caída desde máx(%)', ascending=False)
    top_10_farthest = df_sorted.head(10)
    top_10_closest = df_sorted.tail(10)

    # Formateo de la tabla
    def format_table(df):
        df = df.style.format("{:.1f}", subset=['Caída desde máx (%)', 'Suba potencial (%)', 'Precio máx.', 'Precio actual'])
        df = df.set_properties(**{'text-align': 'center'}, subset=['Caída desde máx (%)', 'Suba potencial (%)', 'Precio máx.', 'Precio actual'])
